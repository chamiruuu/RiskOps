  ...
  const validateAndDetermineIC = () => {
    ...
    if (member_id.includes("@")) {
      ...
      if (matchedMerchant) {
        ...
        if (
          selectedDuty !== "IC0" &&
          matchedMerchant.ic_account !== selectedDuty
        ) {
          return {
            valid: false,
            msg: `Member ID @${code} belongs to ${matchedMerchant.ic_account}, but you are logged in as ${selectedDuty}.`,
          };
        }
    ...
    const selectedMerchantObj = merchants.find(
      (m) => m.merchant_name === merchant_name,
    );
    if (selectedMerchantObj) {
      if (
        selectedDuty !== "IC0" &&
        selectedMerchantObj.ic_account !== selectedDuty
      ) {
        return {
          valid: false,
          msg: `Merchant ${merchant_name} belongs to ${selectedMerchantObj.ic_account}, not your current duty.`,
        };
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">
          Create New Ticket ({selectedDuty})
        </h2>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  member_id: e.target.value.replace(/\s/g, ""),
                })
              }
              required
            />
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  provider_id: e.target.value.replace(/\s/g, ""),
                })
              }
            >
